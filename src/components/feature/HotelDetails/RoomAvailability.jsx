...
const RoomAvailability = ({ rooms, loading, error, hotelId, hotelData }) => {
  ...
  if (error || !availableRooms.length) {
    ...
    const hasRoomsButNoPricing =
      rooms && rooms.length > 0 && !hasAnyPricedRooms;
                ...
                <div className="text-center space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
                        Choose your check-in and check-out dates to see
                        available rooms and pricing for your stay
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base font-semibold rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white hover:from-blue-600 hover:to-blue-700 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl"
                    >
                      <Calendar className="h-3 w-3 sm:h-4 sm:w-4" />
                      <span className="hidden sm:inline">
                        Set Tomorrow as Check-out
                      </span>
                      <span className="sm:hidden">Set Tomorrow</span>
                    </button>

                    <button
                      onClick={() =>
                        router.push(
                          `/hotel/${hotelId}?checkin=${format(checkin, 'yyyy-MM-dd')}&checkout=${format(addDays(checkin, 2), 'yyyy-MM-dd')}`
                        )
                      }
                      className="inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base font-semibold rounded-xl bg-gradient-to-r from-green-500 to-green-600 text-white hover:from-green-600 hover:to-green-700 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl"
                    >
                      <Calendar className="h-3 w-3 sm:h-4 sm:w-4" />
                      <span className="hidden sm:inline">Book 2 Nights</span>
                      <span className="sm:hidden">2 Nights</span>
                    </button>
                  </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <ul className="text-gray-600 space-y-1 text-sm">
                          <li>
                            • See real-time room availability for your specific
                            dates
                          </li>
                          <li>
                            • Get accurate pricing based on your stay duration
                          </li>
          ...
          <div className="text-center mb-8">
            <div className="space-y-4">
              ...
              <p className="text-base text-gray-600 max-w-2xl mx-auto">
                Choose from our selection of comfortable and well-appointed
                rooms for your stay
              </p>
              ...
              <div className="inline-flex items-center gap-4 px-4 py-3 border border-gray-200 rounded-xl text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-700 font-semibold">
                    ...
                    {availableRooms.length !== 1 ? 's' : ''} with pricing
                  </span>
                </div>
                {rooms && rooms.length > availableRooms.length && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-orange-700 font-semibold">
                      {rooms.length - availableRooms.length} room
                      {rooms.length - availableRooms.length !== 1 ? 's' : ''}{' '}
                      priced on request
                    </span>
                  </div>
                )}
            ...
            {availableRooms
              .map((room, index) => {
                ...
                const safeRoom = {
                  ...
                  price_text:
                    room.price_text || room.price || 'Contact for Pricing',
                  ...
                  term_features:
                    Array.isArray(room.term_features) &&
                    room.term_features.length > 0
                      ? room.term_features
                      : [
                ...
                return (
                  <div
                    key={safeRoom.id}
                    className="group bg-white rounded-xl sm:rounded-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-blue-300 transform hover:-translate-y-1 hover:shadow-xl shadow-md"
                  >
                    {/* Enhanced Header with Better Gradient */}
                    <div className="relative bg-gradient-to-br from-blue-600 via-blue-500 to-indigo-600 p-4 sm:p-6 text-white overflow-hidden">
                      {/* Enhanced Decorative Elements */}
                      <div className="absolute top-0 right-0 w-20 h-20 sm:w-32 sm:h-32 bg-white/10 rounded-full -translate-y-12 sm:-translate-y-16 translate-x-12 sm:translate-x-16 animate-pulse"></div>
                      <div
                        className="absolute bottom-0 left-0 w-16 h-16 sm:w-24 sm:h-24 bg-white/5 rounded-full translate-y-8 sm:translate-y-12 -translate-x-8 sm:-translate-x-12 animate-pulse"
                        style={{ animationDelay: '1s' }}
                      ></div>
                      <div
                        className="absolute top-1/2 right-2 sm:right-4 w-8 h-8 sm:w-12 sm:h-12 bg-white/5 rounded-full animate-pulse"
                        style={{ animationDelay: '2s' }}
                      ></div>

                      {/* Room Title */}
                      <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 leading-tight relative z-10">
                        {safeRoom.title}
                      </h3>
                    ...
                    <div className="p-3 sm:p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-4 sm:mb-6">
                        <div className="rounded-lg sm:rounded-xl p-2 sm:p-3 text-center border border-gray-200 hover:border-blue-300 transition-all duration-300 hover:shadow-md">
                            ...
                            {safeRoom.term_features.length > 6 && (
                              <div className="col-span-2 p-2 rounded-lg border border-gray-200 text-center">
                                <span className="text-gray-600 text-xs font-medium">
                                  +{safeRoom.term_features.length - 6} more
                                  amenities available
                                </span>
                              </div>
                            )}
